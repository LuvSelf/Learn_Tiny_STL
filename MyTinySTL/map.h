...
namespace mystl
{
    ...
    /// @tparam Key 键值类型
    /// @tparam T 实值类型
    /// @tparam Compare 键值的比较方式，缺省使用 mystl::less
    ...
    public:
        ...
        mapped_type &operator[](const key_type &key) {
            iterator it = lower_bound(key);
            // it->first >= key
            if (it == end() || key_comp()(key, it->first)) it = emplace_hint(it, key, T{});
            return it->second;
        }
        mapped_type &operator[](key_type &&key) {
            iterator it = lower_bound(key);
            // it->first >= key
            if (it == end() || key_comp()(key, it->first)) it = emplace_hint(it, mystl::move(key), T{});
            return it->second;
        }
    ...
    public:
        ...
        /// @brief functor，用来进行元素比较
        class value_compare : public binary_function<value_type, value_type, bool>
        {
            // 声明为友元，以便multimap可以访问私有构造函数
            friend class multimap<Key, T, Compare>;

        private:
            Compare comp;
            // 私有构造函数，只能由multimap构造
            value_compare(Compare c) : comp(c) {}
