...
namespace mystl
{
    ...
    /// @brief 萃取键值类型
    /// @note 默认处理单一值类型
    /// @note 该模板将 key_type、mapped_type 和 value_type 均定义为 T
    /// @note 如：set<int> 中，键、值均为 int 类型
    template<class T, bool>
    struct rb_tree_value_traits_imp {
        typedef T key_type;   // 键类型
        typedef T mapped_type;// 映射值类型（与键相同）
    ...
    struct rb_tree_node_base {
        ...
        /// @brief 仅当对象为完整节点时安全，若为nil节点，会指向一个不存在的value成员
    ...
    /// @brief 中央类型萃取器
    /// @note 为红黑树提供统一的类型接口，隐藏底层节点实现细节，同时支持 set（单一值）和 map（键值对）两种容器
    template<class T>
    ...
    struct rb_tree_iterator_base : public mystl::iterator<mystl::bidirectional_iterator_tag, T> {
        ...
        // 使迭代器前进
        void inc() {
            if (node->right != nullptr) { node = rb_tree_min(node->right); }
            // 如果没有右子节点
            else
            {
                auto y = node->parent;
        ...
        void dec() {
            ...
            // 左子节点存在时，前驱节点为左子树的最右节点（中序遍历逆序）
            else if (node->left != nullptr) { node = rb_tree_max(node->left); }
            // 非 header 节点，也无左子节点
            else
            {
                auto y = node->parent;
    ...
    mystl::pair<typename rb_tree<T, Compare>::iterator, bool> rb_tree<T, Compare>::emplace_unique(Args &&...args) {
        ...
        // 插入成功时
        if (res.second) { return mystl::make_pair(insert_node_at(res.first.first, np, res.first.second), true); }
        // 键重复时的处理
        // 释放新节点内存
        destroy_node(np);
    ...
    typename rb_tree<T, Compare>::iterator rb_tree<T, Compare>::emplace_multi_use_hint(iterator hint, Args &&...args) {
        ...
        if (node_count_ == 0) { return insert_node_at(header_, np, true); }
        ...
        {
            // 新键小于第一个元素 → 插入最左端
            if (key_comp_(key, value_traits::get_key(*hint))) { return insert_node_at(hint.node, np, true); }
            // 否则正常查找位置
            else
            {
    ...
    /// @return pair，若插入成功，pair 的第二参数为 true，否则为 false
    template<class T, class Compare>
    ...
    /// @brief 插入节点位置查询（允许重复键）
    /// @return 返回一个 pair，包含插入点的父节点和一个 bool 表示是否在左边插入
    template<class T, class Compare>
    ...
    /// @brief 插入节点位置查询（不允许重复键）
    /// @return
    /// 返回一个 pair，第一个值为一个 pair，包含插入点的父节点和一个 bool 表示是否在左边插入
    /// 第二个值为一个 bool，表示是否插入成功
    /// @note
    /// 插入到左子树时，新键必须 > 父节点键，父节点就是直接前驱，只需验证：父节点键 < 新键
    ///
    /// 插入到左子树时，新键必须 < 父节点键，需要验证：前驱节点键 < 新键 < 父节点键
    ///
    /// 特殊情况：无前驱节点时（最小边界）自动满足
    template<class T, class Compare>
    ...
    rb_tree<T, Compare>::get_insert_unique_pos(const key_type &key) {
        ...
        if (add_to_left)
        {
            ...
            if (y == header_ || j == begin()) { return mystl::make_pair(mystl::make_pair(y, true), true); }
            // 否则，如果存在重复节点，那么 --j 就是重复的值，检查前驱节点
            else { --j; }
        }
        // 表明新节点没有重复
        // 严格验证前驱节点键 < 新键
    ...
    template<class T, class Compare>
    typename rb_tree<T, Compare>::iterator rb_tree<T, Compare>::insert_value_at(base_ptr x, const value_type &value,
                                                                                bool add_to_left) {
    ...
    /// @param add_to_left 是否在左边插入
    template<class T, class Compare>
    typename rb_tree<T, Compare>::iterator rb_tree<T, Compare>::insert_node_at(base_ptr x, node_ptr node,
                                                                               bool add_to_left) {
        node->parent = x;
        auto base_node = node->get_base_ptr();
    ...
    /// @brief 在给定提示位置hit附近插入一个允许重复键的节点
    template<class T, class Compare>
    typename rb_tree<T, Compare>::iterator rb_tree<T, Compare>::insert_multi_use_hint(iterator hint, key_type key,
                                                                                      node_ptr node) {
        // 在 hint 附近寻找可插入的位置
        auto np = hint.node;
        auto before = hint;
        ...
        {// before <= node <= hint
            if (bnp->right == nullptr) { return insert_node_at(bnp, node, false); }
            else if (np->left == nullptr) { return insert_node_at(np, node, true); }
        }
        auto pos = get_insert_multi_pos(key);
        return insert_node_at(pos.first, node, pos.second);
    }

    /// @brief 在给定提示位置hit附近插入一个不允许重复键的节点
    template<class T, class Compare>
    typename rb_tree<T, Compare>::iterator rb_tree<T, Compare>::insert_unique_use_hint(iterator hint, key_type key,
                                                                                       node_ptr node) {
        // 在 hint 附近寻找可插入的位置
        auto np = hint.node;
        auto before = hint;
    ...
    /// @brief 递归复制一颗树，节点从 x 开始，p 为 x 的父节点
    template<class T, class Compare>
