...
namespace mystl
{
    ...
    struct pair {
        ...
        pair &operator=(pair &&rhs) noexcept(
                std::is_nothrow_move_assignable<Ty1>::value &&
                std::is_nothrow_move_assignable<Ty2>::value) {
            if (this != &rhs)
            {
        ...
        template<class Other1, class Other2>
        pair &operator=(const pair<Other1, Other2> &other) noexcept(
                std::is_nothrow_assignable<Ty1 &, const Other1 &>::value &&
                std::is_nothrow_assignable<Ty2 &, const Other2 &>::value) {
            if (static_cast<const void *>(&other) != this)
            {
                // 创建副本，如果构造过程中抛出异常，当前对象保持不变
                Ty1 new_first = other.first;
                Ty2 new_second = other.second;
        ...
        pair &operator=(pair<Other1, Other2> &&other) noexcept(
                std::is_nothrow_move_assignable<Ty1 &, Other1 &&>::value &&
                std::is_nothrow_move_assignable<Ty2 &, Other2 &&>::value) {
            // 安全的地址比较
        ...
        // std::declval<ty1&>() 在编译期模拟一个 Ty1 的左值引用
        void swap(pair &other) noexcept(
                noexcept(mystl::swap(std::declval<Ty1 &>(),
                                     std::declval<Ty1 &>())) &&
                noexcept(mystl::swap(std::declval<Ty2 &>(),
                                     std::declval<Ty2 &>()))) {
            if (this != &other)
            {
    ...
    constexpr auto get(const mystl::pair<T1, T2> &p) noexcept
            -> std::conditional_t<I == 0, const T1 &, const T2 &> {
        static_assert(I < 2, "pair index out of bound");
    ...
    template<std::size_t I, class T1, class T2>
    constexpr auto get(mystl::pair<T1, T2> &p) noexcept
            -> std::conditional_t<I == 0, T1 &, T2 &> {
        static_assert(I < 2, "pair index out of bound");
        if constexpr (I == 0) return p.first;
        else
            return p.second;
...
#include <tuple>      // 提供 tuple_size 和 tuple_element
#include <type_traits>// 提供 integral_constant 和 conditional_t
namespace std
{

    template<class T1, class T2>
    struct tuple_size<mystl::pair<T1, T2>> : integral_constant<size_t, 2> {};

    template<size_t I, class T1, class T2>
    struct tuple_element<I, mystl::pair<T1, T2>> {
        static_assert(I < 2, "Index out of bound for pair");
