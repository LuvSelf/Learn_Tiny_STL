...
#define DEQUE_MAP_INIT_SIZE 8
    ...
    struct deque_iterator : public iterator<random_access_iterator_tag, T> {
        ...
        map_pointer node;   // 指向中控器(map)中的节点，该节点指向当前缓冲区
         ...
         * @note 距离计算考虑缓冲区情况
         *
         * node - x.node：计算两个迭代器之间完整的缓冲区数量
         *
         * buffer_size * (node - x.node)：将缓冲区数量转换为元素数量
         *
         * (cur - first)：当前迭代器在它所在缓冲区中的偏移量
         *
         * (x.cur - x.first)：参数迭代器x在其缓冲区中的偏移量
         */
        ...
        self &operator+=(difference_type n) {
            ...
            else
            {
                /*
                  -4 -3 -2 -1 0 1 2 3 4
                  -offset -1是为了将闭区间转为开区间
                  例如：offset=-4，buffer_size = 4，
                  则：若（4/4）= 1，则向前一个缓冲区了
                  若（4-1）/4 = 0，-0-1 = -1，往前一个缓冲区
                */
    ...
    deque<T> &deque<T>::operator=(const deque &rhs) {
        ...
        {
            const auto len = size();
            if (len >= rhs.size()) { erase(mystl::copy(rhs.begin_, rhs.end_, begin_), end_); }
            else
            {
    ...
    void deque<T>::resize(size_type new_size, const value_type &value) {
        const auto len = size();
        if (new_size < len) { erase(begin_ + new_size, end_); }
        else { insert(end_, new_size - len, value); }
    }
    ...
    typename deque<T>::iterator deque<T>::insert(iterator position, const value_type &value) {
        ...
        else { return insert_aux(position, value); }
    }
    ...
    /**
     * @brief 创建并初始化 deque 的中控器（map），分配 size 个指针
     */
    template<class T>
    ...
    // create_buffer 函数
    /**
     * @brief 在 nstart 和 nfinish 之间分配 buffer_size 个元素空间
     */
    template<class T>
    void deque<T>::create_buffer(map_pointer nstart, map_pointer nfinish) {
        ...
        try
        {
            for (cur = nstart; cur <= nfinish; ++cur) { *cur = data_allocator::allocate(buffer_size); }
        }
        catch (...)
        {
            while (cur != nstart)
    ...
    void deque<T>::map_init(size_type nElem) {
        ...
        // 中控器剩余空间 = map_size_ - nNode
        // 左右两侧各预留 (map_size_ - nNode) / 2 个空位
    ...
    // fill_init 函数
    /**
     * @brief 初始化 deque，用 n 个 value 填充
     */
    template<class T>
    ...
    /**
     * @brief 初始化 deque，从 first 到 last 复制元素
     */
    template<class T>
    template<class IIter>
    ...
    /**
     * @brief 将deque 赋值为 [first, last) 范围内的元素
     */
    template<class T>
    template<class IIter>
    void deque<T>::copy_assign(IIter first, IIter last, input_iterator_tag) {
        auto first1 = begin();
        auto last1 = end();
        for (; first != last && first1 != last1; ++first, ++first1) { *first1 = *first; }
        if (first1 != last1) { erase(first1, last1); }
        else { insert_dispatch(end_, first, last, input_iterator_tag{}); }
    }
    ...
    void deque<T>::copy_assign(FIter first, FIter last, forward_iterator_tag) {
        ...
        else { erase(mystl::copy(first, last, begin_), end_); }
    }
    ...
    /**
     * @brief 在 position 位置插入 n 个 value
     */
    template<class T>
    void deque<T>::fill_insert(iterator position, size_type n, const value_type &value) {
        ...
        else
        {
            ...
            {
                if (new_end.node != end_.node) destroy_buffer(end_.node + 1, new_end.node);
                throw;
            }
        }
    }
    ...
    /**
     * @brief 将 [first, last) 插入到 position 位置
     */
    template<class T>
    template<class FIter>
    ...
    void deque<T>::insert_dispatch(iterator position, IIter first, IIter last, input_iterator_tag) {
        ...
        const size_type n = mystl::distance(first, last);
        const size_type elems_before = position - begin_;
        if (elems_before < (size() / 2)) { require_capacity(n, true); }
        else { require_capacity(n, false); }
        position = begin_ + elems_before;
        auto cur = --last;
        for (size_type i = 0; i < n; ++i, --cur) { insert(position, *cur); }
    }
    ...
    void deque<T>::insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag) {
        ...
        else { copy_insert(position, first, last, n); }
    }
    ...
    void deque<T>::require_capacity(size_type n, bool front) {
        ...
        else if (!front && (static_cast<size_type>(end_.last - (end_.cur - 1)) < n))
        {
            ...
            if (need_buffer > static_cast<size_type>(map_size_ - (end_.node - map_ + 1)))
            {
    ...
    void deque<T>::reallocate_map_at_front(size_type need_buffer) {
        ...
        // 复制原缓冲区指针
        for (auto begin1 = mid, begin2 = begin_.node; begin1 != end; ++begin1, ++begin2) *begin1 = *begin2;

        // 更新数据
        map_allocator::deallocate(map_, map_size_);
        map_ = new_map;
        map_size_ = new_map_size;
        // begin_.cur - begin_.first：原元素在缓冲区内的偏移
        // （当前元素，map位置）
    ...
    void deque<T>::reallocate_map_at_back(size_type need_buffer) {
        ...
        for (auto begin1 = begin, begin2 = begin_.node; begin1 != mid; ++begin1, ++begin2) *begin1 = *begin2;
