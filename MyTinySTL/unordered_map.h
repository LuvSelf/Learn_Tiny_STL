...
/**
 * @brief unordered_map.h
 * @note unordered_map 和 unordered_multimap，功能与用法与 map 和 multimap 类似，不同的是使用 hashtable
...
namespace mystl
{
    ...
    /// @tparam Key 键值类型
    /// @tparam T 实值类型
    /// @tparam Hash 哈希函数，缺省使用 mystl::hash
    /// @tparam KeyEqual 键值比较方式，缺省使用 mystl::equal_to
    ...
    public:
        ...
        unordered_map(InputIterator first, InputIterator last, const size_type bucket_count = 100,
        ...
        unordered_map(std::initializer_list<value_type> ilist, const size_type bucket_count = 100,
                      const Hash &hash = Hash(), const KeyEqual &equal = KeyEqual())
    ...
    // 重载 mystl 的 swap
    template<class Key, class T, class Hash, class KeyEqual>
    void swap(unordered_map<Key, T, Hash, KeyEqual> &lhs, unordered_map<Key, T, Hash, KeyEqual> &rhs) {
        lhs.swap(rhs);
    }
     ...
     * @param Key 键值类型
     * @param T 实值类型
     * @param Hash 哈希函数，缺省使用 mystl::hash
     * @param KeyEqual 键值比较方式，缺省使用 mystl::equal_to
     */
    template<class Key, class T, class Hash = mystl::hash<Key>, class KeyEqual = mystl::equal_to<Key>>
    ...
    public:
        ...
        template<class InputIterator>
        unordered_multimap(InputIterator first, InputIterator last, const size_type bucket_count = 100,
                           const Hash &hash = Hash(), const KeyEqual &equal = KeyEqual())
            : ht_(mystl::max(bucket_count, static_cast<size_type>(mystl::distance(first, last))), hash, equal) {
            for (; first != last; ++first) ht_.insert_multi_noresize(*first);
        }

        unordered_multimap(std::initializer_list<value_type> ilist, const size_type bucket_count = 100,
                           const Hash &hash = Hash(), const KeyEqual &equal = KeyEqual())
            : ht_(mystl::max(bucket_count, static_cast<size_type>(ilist.size())), hash, equal) {
            for (auto first = ilist.begin(), last = ilist.end(); first != last; ++first)
