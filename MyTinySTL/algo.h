...
namespace mystl
{
    ...
    /**
     * @brief  在[first1, last1)中查找满足comp条件的[first2, last2)的首次出现点
     * @return 找不到返回last1
     */
    ...
    ForwardIter search_n(ForwardIter first, ForwardIter last, Size n, const T &value) {
        ...
        else
        {
            ...
            while (first != last)
            {
                ...
                if (m == 0) { return first; }
                else { first = mystl::find(i, last, value); }
            }
            return last;
        }
    }
    ...
    /**
     * @brief  在[first, last)中查找连续满足 comp 所形成的子序列，返回一个迭代器指向该子序列的起始处
     * @return 找不到返回last，子串长度为0返回first
     */
     ...
     * @note find_end_dispatch 的 forward_iterator_tag 版本
     */
    template<class ForwardIter1, class ForwardIter2>
    ...
    /**
     * @brief  在[first1, last1)区间中查找[first2, last2)最后一次出现的地方，若不存在返回 last1
     * @note find_end_dispatch 的 bidirectional_iterator_tag 版本
     */
    template<class BidirectionalIter1, class BidirectionalIter2>
    BidirectionalIter1 find_end_dispatch(BidirectionalIter1 first1, BidirectionalIter1 last1, BidirectionalIter2 first2,
                                         BidirectionalIter2 last2, bidirectional_iterator_tag,
                                         bidirectional_iterator_tag) {
        typedef reverse_iterator<BidirectionalIter1> reviter1;
        typedef reverse_iterator<BidirectionalIter2> reviter2;
        reviter1 rlast1(first1);// 指向 first1 的反向迭代器
        reviter2 rlast2(first2);// 指向 first2 的反向迭代器

        // 在反转序列中搜索
        reviter1 rresult = mystl::search(reviter1(last1),// 主序列尾部(反转后变成开头)
                                         rlast1,         // 主序列开头(反转后变成结尾)
                                         reviter2(last2),// 子序列尾部(反转后变成开头)
                                         rlast2);        // 子序列开头(反转后变成结尾)
    ...
    /**
     * @brief  在[first1, last1)中查找[first2, last2)中的任意一元素，返回指向第一次出现的元素的迭代器
     * @return 找不到返回 last1
     */
    ...
    /**
     * @brief  在[first1, last1)中查找[first2, last2)中的满足条件的任意一元素，返回指向第一次出现的元素的迭代器
     * @return 找不到返回 last1
     */
    ...
    /**
     * @brief  用一个函数对象 f 对[first, last)区间内的每个元素执行一个 operator() 操作，但不能改变元素内容
     * @note f() 可返回一个值，但该值可能会被忽略
     */
    ...
    /**
     * @brief  找出第一对匹配的相邻元素，使用函数对象 comp 代替比较，如果找到返回一个迭代器，指向这对元素的第一个元素
     * @return 失败返回last
     */
    ...
    /**
     * @brief  在升序序列[first, last)中查找第一个不小于 value 的元素，并返回指向它的迭代器
     * @return 失败返回last
     * @note lbound_dispatch 的 forward_iterator_tag 版本
     */
     ...
     * @brief  在升序序列[first, last)中查找第一个不小于 value 的元素，并返回指向它的迭代器
     * @return 失败返回last
     * @note lbound_dispatch 的 random_access_iterator_tag 版本
     */
    ...
    /**
     * @brief  在有序序列[first, last)中查找第一个不小于 value 的元素，并返回指向它的迭代器
     * @return 失败返回last
     * @note lbound_dispatch 的 forward_iterator_tag 版本
     * @note comp(a, b) = (a < b)时升序，comp(a, b) = (a > b)时降序
     */
    ...
    ForwardIter lbound_dispatch(ForwardIter first, ForwardIter last, const T &value, forward_iterator_tag,
        ...
        while (len > 0)
        {
            ...
            middle = first;
            mystl::advance(middle, half);
            if (comp(*middle, value))//*middle 应排在 value 前
            {
                first = middle;
                ++first;
                len = len - half - 1;
        ...
        return first;// 首个满足 !comp(*it, value) 的位置
    }
    ...
    /**
     * @brief  在有序序列[first, last)中查找第一个大于value 的元素，并返回指向它的迭代器
     * @return 失败返回last
     * @note ubound_dispatch 的 forward_iterator_tag 版本
     */
    template<class ForwardIter, class T>
     ...
     * @brief  在有序序列[first, last)中查找第一个大于value 的元素，并返回指向它的迭代器
     * @return 失败返回last
     * @note ubound_dispatch 的 random_access_iterator_tag 版本
     */
    template<class RandomIter, class T>
    ...
    /**
     * @brief  在有序序列[first, last)中查找第一个满足comp 的元素，并返回指向它的迭代器
     * @return 失败返回last
     * @note ubound_dispatch 的 forward_iterator_tag 版本
     * @note comp为true时，value在左半边
     */
    template<class ForwardIter, class T, class Compared>
    ForwardIter ubound_dispatch(ForwardIter first, ForwardIter last, const T &value, forward_iterator_tag,
                                Compared comp) {
        ...
        while (len > 0)
        {
            ...
            middle = first;
            mystl::advance(middle, half);
            if (comp(value, *middle)) { len = half; }//*middle 应排在 value 后
            else
            {
                first = middle;
                ++first;
    ...
    /**
     * @brief  二分查找，如果在有序序列[first, last)内有等同于 value 的元素，返回 true
     * @return 失败返回 false
     */
    template<class ForwardIter, class T>
    ...
    bool binary_search(ForwardIter first, ForwardIter last, const T &value, Compared comp) {
        ...
        // lower_bound 已保证 !comp(*i, value)（即 *i 不小于 value）
        //  /! comp(value, *i)检查value是否不位于*i之前
        // 两者结合即为等价
    ...
    /**
     * @brief  查找升序区间[first,last)区间中所有等于value的元素范围，返回一对迭代器指向区间首尾
     * @return 第一个迭代器指向第一个不小于 value 的元素，第二个迭代器指向第一个大于 value 的元素
     * @note erange_dispatch 的 forward_iterator_tag 版本
     */
    template<class ForwardIter, class T>
    mystl::pair<ForwardIter, ForwardIter> erange_dispatch(ForwardIter first, ForwardIter last, const T &value,
        ...
        while (len > 0)
        {
            ...
            {
                // 由于区间内至少有一个等于value，所以left会指向第一个等于value的元素
                left = mystl::lower_bound(first_s, last_s, value);
                // mystl::advance(first, len);
                //  第一个大于value的元素
                right = mystl::upper_bound(++middle, last_s, value);
    ...
    /**
     * @brief  查找升序区间[first,last)区间中与 value 相等的元素所形成的区间，返回一对迭代器指向区间首尾
     * @return 第一个迭代器指向第一个不小于 value 的元素，第二个迭代器指向第一个大于 value 的元素
     * @note erange_dispatch 的 random_access_iterator_tag 版本
     */
    template<class RandomIter, class T>
    mystl::pair<RandomIter, RandomIter> erange_dispatch(RandomIter first, RandomIter last, const T &value,
    ...
    /**
     * @brief  查找有序序列[first,last)区间中与 value 相等的元素所形成的区间，返回一对迭代器指向区间首尾
     * @return 第一个迭代器指向第一个不小于 value 的元素，第二个迭代器指向第一个大于 value 的元素
     * @note erange_dispatch 的 forward iterator 版本
     * @note 函数对象 comp 代替比较操作
     */
    template<class ForwardIter, class T, class Compared>
    mystl::pair<ForwardIter, ForwardIter> erange_dispatch(ForwardIter first, ForwardIter last, const T &value,
     ...
     * @brief  查找有序序列[first,last)区间中与 value 相等的元素所形成的区间，返回一对迭代器指向区间首尾
     * @return 第一个迭代器指向第一个不小于 value 的元素，第二个迭代器指向第一个大于 value 的元素
     * @note erange_dispatch 的 random access iterator 版本
     * @note 函数对象 comp 代替比较操作
     */
    template<class RandomIter, class T, class Compared>
    mystl::pair<RandomIter, RandomIter> erange_dispatch(RandomIter first, RandomIter last, const T &value,
    ...
    /**
     * @brief  检查[first, last)内的元素是否有序，如果是有序，则返回 true
     */
    template<class ForwardIter, class Compared>
     ...
     * @note 交换的区间长度必须相同，两个序列不能互相重叠，返回一个迭代器指向序列二最后一个被交换元素的下一位置
     */
    template<class ForwardIter1, class ForwardIter2>
    ForwardIter2 swap_ranges(ForwardIter1 first1, ForwardIter1 last1, ForwardIter2 first2) {
        for (; first1 != last1; ++first1, ++first2) { mystl::iter_swap(first1, first2); }
        return first2;
    }
    ...
    /**
     * @brief  以函数对象 binary_op 作用于两个序列[first1, last1)、[first2, last2)的相同位置，将结果保存至 result 中
     */
    template<class InputIter1, class InputIter2, class OutputIter, class BinaryOperation>
    OutputIter transform(InputIter1 first1, InputIter1 last1, InputIter2 first2, OutputIter result,
    ...
    /**
     * @brief  移除区间内与指定 value 相等的元素，并将结果复制到以 result 标示起始位置的容器上
     * @note 不影响原容器，只是将不等于value的元素按序复制到result中
     */
     ...
     * @note 逻辑删除，将不等于 value 的元素移动到前面，并返回新的 last 位置，容器大小不变
     */
    template<class ForwardIter, class T>
     ...
     * @note 并不从容器中删除这些元素，所以 remove 和 remove_if 不适用于 array
     * @note 逻辑删除，将不为 true 的元素移动到前面，并返回新的 last 位置，容器大小不变
     */
     ...
     */
    template<class InputIter, class OutputIter, class T>
    OutputIter replace_copy(InputIter first, InputIter last, OutputIter result, const T &old_value,
                            const T &new_value) {
     ...
     * @note 通过函数对象 unary_pred 判断元素是否需要替换
     */
    ...
    OutputIter replace_copy_if(InputIter first, InputIter last, OutputIter result, UnaryPredicate unary_pred,
                               const T &new_value) {
        for (; first != last; ++first, ++result) { *result = unary_pred(*first) ? new_value : *first; }
        return result;
    }
    ...
    /**
     * @brief 将[first, last)区间内的元素反转，将结果复制到 result 所指容器中
     * @note 迭代器类型匹配
     */
    ...
    void random_shuffle(RandomIter first, RandomIter last) {
        ...
        for (auto i = first + 1; i != last; ++i) { mystl::iter_swap(i, first + (rand() % (i - first + 1))); }
    }

    /**
     * @brief 将[first, last)内的元素次序随机重排
     * @note 使用一个产生随机数的函数对象 rand
     */
    ...
    void random_shuffle(RandomIter first, RandomIter last, RandomNumberGenerator &rand) {
        if (first == last) return;
        auto len = mystl::distance(first, last);
        for (auto i = first + 1; i != last; ++i) { mystl::iter_swap(i, first + (rand(i - first + 1) % len)); }
    }

    /**
     * @brief 将[first, middle)内的元素和 [middle, last)内的元素互换，可以交换两个长度不同的区间
     * @return 返回交换后 middle 的位置
    ...
    RandomIter rotate_dispatch(RandomIter first, RandomIter middle, RandomIter last, random_access_iterator_tag) {
        ...
        {
            auto tmp = *first;// 保存起始元素
            auto p = first;   // 当前操作位置指针
            if (l < r)
            {
    ...
    /**
     * @brief 行为与 rotate 类似，不同的是将结果复制到 result 所指的容器中
     * @return 返回交换后 middle 的位置
     */
     ...
     * @note 如果当前排列不是最大排列，生成下一个更大的排列并返回 true
     * @note 如果当前已是最大排列，将序列重置为最小排列并返回 false
     */
    template<class BidirectionalIter>
     ...
     * @note 如果当前排列不是最小排列，生成下一个更小的排列并返回 true
     * @note 如果当前已是最小排列，将序列重置为最大排列并返回 false
     */
    template<class BidirectionalIter>
    ...
    void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last, Distance len1,
                              Distance len2) {
        // 边界情况处理
    ...
    /**
     * @brief 将两个已排序的序列 [first1, last1) 和 [first2, last2) 从后向前合并到目标位置 result
     */
    template<class BidirectionalIter1, class BidirectionalIter2>
    BidirectionalIter1 merge_backward(BidirectionalIter1 first1, BidirectionalIter1 last1, BidirectionalIter2 first2,
    ...
    /**
     * @brief 自适应旋转操作，根据两个序列的长度和缓冲区大小选择最优的旋转策略：
     * @note 优先使用缓冲区加速旋转
     * @note 当缓冲区不足时回退到标准旋转算法
     * @note 最小化元素移动次数
     */
    ...
    BidirectionalIter1 rotate_adaptive(BidirectionalIter1 first, BidirectionalIter1 middle, BidirectionalIter1 last,
        ...
        // 情况3：缓冲区不足，使用标准旋转
        else { return mystl::rotate(first, middle, last); }
    }
    ...
    void merge_adaptive(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last, Distance len1,
        ...
        else
        {
            ...
            // 自适应旋转
            auto new_middle =
                    mystl::rotate_adaptive(first_cut, middle, second_cut, len1 - len11, len22, buffer, buffer_size);
            // 递归合并子序列
    ...
    void inplace_merge_aux(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last, T *) {
        ...
        // 缓冲区申请失败
        if (!buf.begin()) { mystl::merge_without_buffer(first, middle, last, len1, len2); }
        // 使用缓冲区优化
        else { mystl::merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size()); }
    }
    ...
    void inplace_merge_aux(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last, T *,
        ...
        auto len2 = mystl::distance(middle, last);
        temporary_buffer<BidirectionalIter, T> buf(first, last);
        if (!buf.begin()) { mystl::merge_without_buffer(first, middle, last, len1, len2, comp); }
        else { mystl::merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size(), comp); }
    }
    ...
    void partial_sort(RandomIter first, RandomIter middle, RandomIter last) {
        ...
        {
            if (*i < *first) { mystl::pop_heap_aux(first, middle, i, *i, distance_type(first)); }
        }
    ...
    void partial_sort(RandomIter first, RandomIter middle, RandomIter last, Compared comp) {
        ...
        for (auto i = middle; i < last; ++i)
        {
            if (comp(*i, *first)) { mystl::pop_heap_aux(first, middle, i, *i, distance_type(first), comp); }
        }
        mystl::sort_heap(first, middle, comp);
    }

    /**
     * @brief 行为与 partial_sort 类似，不同的是把排序结果复制到 result 容器中
     */
    ...
    mystl::pair<OutputIter1, OutputIter2> partition_copy(InputIter first, InputIter last, OutputIter1 result_true,
    ...
    void unchecked_insertion_sort(RandomIter first, RandomIter last) {
        for (auto i = first; i != last; ++i) { mystl::unchecked_linear_insert(i, *i); }
    }
    ...
    void insertion_sort(RandomIter first, RandomIter last) {
        ...
        for (auto i = first + 1; i != last; ++i)
        {
            ...
            else { mystl::unchecked_linear_insert(i, value); }
        }
    }
    ...
    void final_insertion_sort(RandomIter first, RandomIter last) {
        ...
        else { mystl::insertion_sort(first, last); }
    }
    ...
    void unchecked_insertion_sort(RandomIter first, RandomIter last, Compared comp) {
        for (auto i = first; i != last; ++i) { mystl::unchecked_linear_insert(i, *i, comp); }
    }
    ...
    void insertion_sort(RandomIter first, RandomIter last, Compared comp) {
        ...
        for (auto i = first + 1; i != last; ++i)
        {
            ...
            else { mystl::unchecked_linear_insert(i, value, comp); }
        }
    }
    ...
    void final_insertion_sort(RandomIter first, RandomIter last, Compared comp) {
        ...
        else { mystl::insertion_sort(first, last, comp); }
    }
