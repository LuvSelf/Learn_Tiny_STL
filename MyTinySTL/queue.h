...
namespace mystl
{
    ...
    /// @brief 模板类 queue
    /// @tparam T 数据类型
    /// @tparam Container 底层容器类型，缺省使用 mystl::deque 作为底层容器
    ...
    public:
        ...
        queue(queue &&rhs) noexcept(std::is_nothrow_move_constructible<Container>::value) : c_(mystl::move(rhs.c_)) {}
    ...
    /// @brief 模板类 priority_queue优先级队列
    /// @tparam T 数据类型
    /// @tparam Container 底层容器类型，缺省使用 mystl::vector 作为底层容器
    /// @tparam Compare 权值比较的标准，缺省使用 mystl::less 作为比较方式
    /// @note 所以修改操作后调用堆算法维护堆结构
    ...
    public:
        ...
        priority_queue(const Compare &com) : c_(), comp_(com) {}

        explicit priority_queue(size_type n) : c_(n) { mystl::make_heap(c_.begin(), c_.end(), comp_); }
        ...
        priority_queue(std::initializer_list<T> ilist) : c_(ilist) { mystl::make_heap(c_.begin(), c_.end(), comp_); }

        priority_queue(const Container &s) : c_(s) { mystl::make_heap(c_.begin(), c_.end(), comp_); }
        priority_queue(Container &&s) : c_(mystl::move(s)) { mystl::make_heap(c_.begin(), c_.end(), comp_); }
